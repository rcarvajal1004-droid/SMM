...
export class HomeComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  readonly locationName = BUSINESS_CONTACT.name;
  readonly locationCoordinates = BUSINESS_CONTACT.coordinates;
  readonly mapsUrl = BUSINESS_CONTACT.mapsUrl;
  readonly mapsEmbedUrl: SafeResourceUrl = this.sanitizer.bypassSecurityTrustResourceUrl(BUSINESS_CONTACT.mapsEmbedUrl);
  ...
  private mouseFrame: number | null = null;
  private pendingMouse: { event: MouseEvent; card: HTMLElement } | null = null;
  private marqueeObserver: IntersectionObserver | null = null;
  ...
  ngOnDestroy(): void {
    ...
    if (this.mouseFrame !== null) cancelAnimationFrame(this.mouseFrame);
    this.marqueeObserver?.disconnect();
  }

  onMouseMove(e: MouseEvent, card: EventTarget | null): void {
    if (!this.isBrowser || this.animations.reducedMotion) return;
    if (!(card instanceof HTMLElement)) return;
    this.pendingMouse = { event: e, card };
    if (this.mouseFrame !== null) return;
    this.mouseFrame = requestAnimationFrame(() => {
      const pending = this.pendingMouse;
      this.mouseFrame = null;
      if (!pending) return;
      const rect = pending.card.getBoundingClientRect();
      pending.card.style.setProperty('--mouse-x', `${pending.event.clientX - rect.left}px`);
      pending.card.style.setProperty('--mouse-y', `${pending.event.clientY - rect.top}px`);
    });
  }
